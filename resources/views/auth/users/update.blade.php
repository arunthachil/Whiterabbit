    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ url('/users/update/'.$id) }}">
                        @csrf
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                            <select name="role" id="role" required class="form-control @error('email') is-invalid @enderror" aria-label="Default select example">
                                <option selected>Select Role</option>
                                @foreach($data['roles'] as $key => $role)
                                    <option {{ $role['id'] == $data['user']['role_id'] ? 'selected' : '' }} value="{{$role['id']}}">{{$role['name']}}</option>
                                @endforeach
                            </select>
